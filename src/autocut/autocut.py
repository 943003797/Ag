...
from pyJianYingDraft import tim, TextIntro, TextOutro,TextLoopAnim,AudioSceneEffectType, Text_loop_anim, Mask_type, VideoSceneEffectType, animation, IntroType, OutroType, Transition_type, trange, GroupAnimationType, TransitionType
...
class autoCut():
    def __init__(self, topic: str = "", bgm: str = "落.mp3"):
        ...
        self.script.add_track(draft.TrackType.video, 'Video', mute= True, relative_index=0)
        ...
        self.script.add_track(draft.TrackType.audio, 'Audio')
        self.script.add_track(draft.TrackType.text, 'SubTitle')
        self.script.add_track(draft.TrackType.video, 'SubTitleBKG')
    ...
    def addVideo(self):
      ...
      video_segment = draft.VideoSegment(material = video_material, target_timerange = trange(0, int(self.duration)), volume=2.5)
      # 效果
      video_segment.add_animation(IntroType.砸出波纹, tim('2s'))
      self.addSound(sound='空灵水滴.mp3', start_time=0, track='Sound')
      video_segment.add_animation(OutroType.渐隐, tim('1s'))
    #   video_segment.add_effect(VideoSceneEffectType.低音增强)
      self.script.add_segment(video_segment, 'Video')
    
    def addAudio(self):
      audio_material = draft.AudioMaterial(self.draft_dir + "/Resources/audioAlg/audio.mp3")
      self.script.add_material(audio_material)
      audio_segment = draft.AudioSegment(material = audio_material, target_timerange = trange(0, int(self.duration)), volume=2.0)
      audio_segment.add_effect(AudioSceneEffectType.低音增强, [30])
      self.script.add_segment(audio_segment, 'Audio')
    ...
    def addWaterMark(self):
        ...
        WaterMark_Segment.add_animation(TextIntro.开幕, 1500000)
    ...
    def addBgm(self):
        audio_bgm = draft.AudioMaterial(self.material + "bgm/" + self.bgm)
        self.script.add_material(audio_bgm)
        audio_bgm_segment = draft.AudioSegment(audio_bgm, trange(0, self.duration),volume=0.75)
        audio_bgm_segment.add_fade("0.2s", "1s")
        audio_bgm_segment.add_effect(AudioSceneEffectType.低音增强, [15])
    ...
    def addItem(self) -> str:
        for key,item in enumerate(self.list):
            ...
            if os.path.exists(f"{item['audio_patch']}"):
                ...
                #       TextSegment.add_animation(TextOutro.渐隐, 1000000)
                #       self.addSound(sfx="0点5呼.mp3", track='SFX')
                #     case 'left':
                #       item['danmu'] = f"""{item['danmu']}"""
                #       # 统计item['danmu']中的换行符数量
                #       newline_count = item['danmu'].count('\n')
        ...
        return 'Success'
    def addSound(self, sound: str = "0点5呼.mp3", start_time: int = None, track: str = 'Sound'):
        if os.path.exists(os.path.join(self.material + 'sound/', sound)):
          SoundMaterial = draft.AudioMaterial(os.path.join(self.material + 'sound/', sound))
          sound_length = SoundMaterial.duration
          self.script.add_material(SoundMaterial)
          SoundSegment = draft.AudioSegment(SoundMaterial,
                          trange(int(start_time), int(sound_length)),
                          volume=1.8)
          self.script.add_segment(SoundSegment, track)
    
    def addSubtitle(self):
        """
        读取ASR结果中的sentences，遍历words，当遇到punctuation不为空时，
        将该子句作为一个字幕添加到SubTitle轨道
        """
        asr_path = self.draft_dir + "Resources/data/asr.json"
        if not os.path.exists(asr_path):
            print("未找到ASR文件，跳过字幕添加")
            return
        ...
        with open(asr_path, "r", encoding="utf-8") as f:
            try:
                ...
                print("asr.json 格式错误，跳过字幕添加")
                return
        
        sentences = asr_data.get("text", [])
        sentences = sentences.replace("'", '"')
        sentences = json.loads(sentences)
        sentences = sentences['sentences']
        if not sentences:
            print("ASR文件中没有sentences数据，跳过字幕添加")
            return
        ...
        for sentence in sentences:
            ...
            for word in words:
                ...
                if sub_begin_time is None:
                    sub_begin_time = begin_time
                
                current_subtitle.append(text)
                sub_end_time = end_time
                
                if punctuation:
                    subtitle_text = "".join(current_subtitle)
                    begin_time_ms = int(sub_begin_time * 1000)
                    duration_ms = int((sub_end_time - sub_begin_time) * 1000)
                    
                    TextSegment = draft.TextSegment(
                        subtitle_text,
                        trange(begin_time_ms, duration_ms),
                        font=draft.FontType.三极黑宋体中粗,                                  # 设置字体为文轩体
                        style=draft.TextStyle(color=(1, 1, 1), size=5, letter_spacing  = 1),                # 设置字体颜色为黄色
                        # border=draft.TextBorder(color=(0, 0, 0)),
                        shadow=draft.TextShadow(color=(0, 0, 0),alpha=0.6,diffuse = 5,distance = 4,angle = -12),
                        clip_settings=draft.ClipSettings(transform_y=-0.8)
                    )
                    TextSegment.add_animation(TextIntro.渐显, tim('0.1s'))
                    TextSegment.add_animation(TextOutro.渐隐, tim('0.1s'))
                    self.script.add_segment(TextSegment, 'SubTitle')
                    # 字幕条
                    video_material = draft.VideoMaterial(self.material + "image/24.png")
                    self.duration = video_material.duration
                    self.script.add_material(video_material)
                    video_segment = draft.VideoSegment(material = video_material, target_timerange = trange(begin_time_ms, duration_ms), volume=2.5,
                                                        clip_settings=draft.ClipSettings(
                                                          transform_x=0,
                                                          transform_y=-0.8,
                                                           scale_x=1,
                                                            scale_y=1))
                    video_segment.add_animation(IntroType.渐显, tim('0.1s'))
                    video_segment.add_animation(OutroType.渐隐, tim('0.1s'))
                    self.script.add_segment(video_segment, 'SubTitleBKG')
    ...
    def addKeyWord(self):
        ...
        """
        keyword_path = self.draft_dir + "Resources/data/keyword.json"
        ...
        for key, item in enumerate(keywords_data):
          ...
          match item['type']:
            case 0:#middle
              ...
              TextSegment.add_animation(TextIntro.辉光, 1000000)
              TextSegment.add_animation(TextOutro.渐隐, 1000000)
              self.addSound(sound="高音闪光转场.mp3", start_time=item['begin_time'])
              self.script.add_segment(TextSegment, 'T0')
            case 1:#right
              ...
              TextSegment.add_animation(TextOutro.渐隐, 1000000)
              self.addSound(sound="0点5呼.mp3", start_time=item['begin_time'])
              self.script.add_segment(TextSegment, 'T1')
            case 2:
              item['keyword'] = f"""{item['keyword']}"""
              ...
              TextSegment = draft.TextSegment(item['keyword'], trange(int(item['begin_time']), int((item['end_time']-item['begin_time'])) + self.danmu_extend_ms),  # 文本将持续整个视频（注意script.duration在上方片段添加到轨道后才会自动更新） 
                              font=draft.FontType.三极行楷简体_粗,
                            #   border=draft.TextBorder(color=(0.172, 0.184, 0.231)),
                              shadow=draft.TextShadow(color=(0, 0, 0),alpha=1,diffuse = 5, distance = 5,angle = -20),
                              style=draft.TextStyle(
                                color=(1, 0.949, 0),
                                size=9,
                                align=3,
                              ...
                              # shadow=draft.TextShadow(color=(0, 0, 0),alpha=0.8,diffuse = 15),
                              # border=draft.TextBorder(color=(0, 0, 0)),
                              clip_settings=draft.ClipSettings(transform_y=-0.37, transform_x=0))          # 模拟字幕的位置
              TextSegment.add_animation(TextIntro.放大, tim('0.2s'))
              TextSegment.add_animation(TextOutro.渐隐, tim('0.2s'))
              self.addSound(sound="0点5呼.mp3", start_time=item['begin_time'])
              self.script.add_segment(TextSegment, 'T2')
            case 'left':
              item['keyword'] = f"""{item['keyword']}"""
              # 统计item['keyword']中的换行符数量
              newline_count = item['keyword'].count('\n')
    ...
    def general_draft(self):
        try:
            ...
            self.addSubtitle()
            self.addKeyWord()
            self.script.dump(self.draft_dir + '/draft_content.json')

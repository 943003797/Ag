...
class LLM:
    ...
    def get_cover_prompt(self, topic: str) -> str:
        """
        根据主题生成封面图片的Prompt
        ...
        """
        try:
            content = f'根据主题"{topic}"，生成封面图片的prompt。要求：\n1. 描述一个适合作为视频封面的中国风场景\n2. 包含具体的环境、人物姿态、光线等细节\n3. 字数在50-100字之间\n4. 输出纯文本描述，不要包含任何格式符号或JSON'
            response = self.client.chat.completions.create(
                model=self.model,
                messages=[
                    {
            ...
            print(f"[ERROR] LLM.get_cover_prompt 出错: {e}")
